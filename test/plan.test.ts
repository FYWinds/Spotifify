...
describe("computePlaylistPlan", () => {
  ...
  test("splits stale items into prune (managed) and foreign, keeps them at the tail when prune is off", () => {
    const remote = [t("a"), t("b"), t("x"), t("y")].map((uri) => ({ uri, isLocal: false, stale: false }));
  ...
  test("with prune enabled the target order excludes pruned items and adds are appended before reordering", () => {
    const remote = [t("a"), t("x"), t("b")].map((uri) => ({ uri, isLocal: false, stale: false }));
    const p = computePlaylistPlan(
  ...
  test("duplicate remote items: first occurrence follows source order, extras trail as foreign", () => {
    const remote = [t("a"), t("a"), t("b")].map((uri) => ({ uri, isLocal: false, stale: false }));
  ...
  test("replace is disallowed when any local item is involved", () => {
    const withLocal = computePlaylistPlan(base({ remote: [{ uri: "spotify:local:a:b:c", isLocal: true, stale: false }] }));
    expect(withLocal.replaceAllowed).toBe(false);
    const without = computePlaylistPlan(base({ remote: [{ uri: t("a"), isLocal: false, stale: false }] }));
    expect(without.replaceAllowed).toBe(true);
  });

  test("stale local entries are pruned even though no local item is ever managed", () => {
    const stale = { uri: "spotify:local:a:b:c+(local):120", isLocal: true, stale: true };
    const p = computePlaylistPlan(base({ desired: [item("spotify:local:a:b:c", "local")], remote: [{ uri: t("z"), isLocal: false, stale: false }, stale] }));
    expect(p.prune).toEqual([{ uri: stale.uri, positions: [1] }]);
    expect(p.foreign).toEqual([t("z")]);
    expect(p.awaiting.map((a) => a.uri)).toEqual(["spotify:local:a:b:c"]);
  });
...
describe("resolveRemoteLocalUri", () => {
  ...
  test("a client entry with the same identity resolves to the export record whatever its percent-encoding", () => {
    expect(resolveRemoteLocalUri("spotify:local:%E5%91%A8%E6%9D%B0%E4%BC%A6:%E5%8F%B6%E6%83%A0%E7%BE%8E:%E6%99%B4%E5%A4%A9:269", exports)).toEqual({ uri: exported, stale: false });
  });

  test("wrong identities of our own export (other duration, `:0` from a bare paste, no duration, title suffix) are stale", () => {
    for (const wrong of [
      ...
      expect(resolveRemoteLocalUri(wrong, exports)).toEqual({ uri: wrong, stale: true });
    }
  });
  ...
  test("unrelated local files stay foreign, non-local URIs pass through", () => {
    ...
    expect(resolveRemoteLocalUri(other, exports)).toEqual({ uri: other, stale: false });
    expect(resolveRemoteLocalUri(t("a"), exports)).toEqual({ uri: t("a"), stale: false });
  });
});
